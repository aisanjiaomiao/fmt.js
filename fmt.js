...
// 是否dayjs 对象
const isDayjsObject = v => (typeof v == 'object' ? dayjs().constructor === v.constructor : false)
// 是否Moment对象
const isMomentObject = v => v._isAMomentObject
// 是否ISO_8601日期格式字符串
const isIso8601 = v => typeof v == 'string' && v.trim().length > 19 && /^\d{4}-\d\d-\d\dT\d\d:\d\d:\d\d(\.\d+)?(([+-]\d\d:\d\d)|Z)?$/i.test(v)
// 是否空对象
...
// 是否空
const isEmpty = v => [undefined, null].includes(v) || (typeof v == 'number' && isNaN(v)) || (typeof v == 'string' && !v.trim()) || (Array.isArray(v) && !v.length)
/**
 * @desc  模板引擎
 ...
 * @desc  根据函数格式化  
 */
const formatMethods = new Proxy({}, { get: (target, prop, receiver) => findMethod(prop) })
/* 格式化方式匹配 */
const matchFormatMethod = {
    /**
     * @desc  根据函数格式化
     * @param {Any}      oVal     原始数据
